...
// ★ 追加: 角度確認用変数（度数法）
double avg_theta_deg = 0.0;
double mol0_theta1_deg = 0.0;
double mol0_theta2_deg = 0.0;
...
void minimize_steepest_descent(); // SD法のプロトタイプ宣言
...
int main() {
    ...
    for (int step = 1; step <= NSTEPS; step++) {
        integrate_step1(); // 座標の更新と速度の半ステップ更新
        compute_forces();  // 新しい座標での力とポテンシャルエネルギーを計算
        integrate_step2(); // 速度の残り半ステップ更新

        // ★ 追加: 毎ステップ、系全体の平均結合角と分子0の結合角を出力
        printf("Step: %5d | Avg Theta: %6.2f deg | Mol0 Theta: %.2f deg, %.2f deg\n",
               step, avg_theta_deg, mol0_theta1_deg, mol0_theta2_deg);
...
void minimize_steepest_descent() {
    ...
    double alpha = 0.0005;              // 歩幅 (少し慎重な値に変更)
    double pe_prev = 1e10;
...
void compute_forces() {
    ...
    ep_lj = ep_bond = ep_angle = ep_dih = 0.0;

    // 1. Lennard-Jones力 (分子間相互作用)
    ...
    for (int i = 0; i < NATOMS - 1; i++) {
        for (int j = i + 1; j < NATOMS; j++) {
            if (atoms[i].mol_id == atoms[j].mol_id) continue;
    ...
    for (int m = 0; m < NMOL; m++) {
        ...
        for (int i = 0; i < SITES_PER_MOL - 2; i++) {
            ...
            sum_theta += theta;
            count_theta++;
            if (m == 0 && i == 0) mol0_theta1_deg = theta * (180.0 / M_PI);
            if (m == 0 && i == 1) mol0_theta2_deg = theta * (180.0 / M_PI);
        ...
        for (int i = 0; i < SITES_PER_MOL - 3; i++) {
            ...
            if (m_sq < 1e-10 || n_sq < 1e-10) continue;
    ...
    // ★ 追加: 全体の平均角度（度）を計算
    if (count_theta > 0) {
        avg_theta_deg = (sum_theta / count_theta) * (180.0 / M_PI);
    }
}
