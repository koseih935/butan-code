...
void minimize_steepest_descent() {
    ...
    for (int step = 0; step < MAX_STEPS; step++) {
        // 1. 本番用の関数を呼んで Particle atoms の力を計算
        compute_forces();
        compute_energy_and_temp(); // これを呼ぶことで e_pot が最新化される
        ...
        // 3. 歩幅（アルファ）の自動調整
        if (e_pot > pe_prev) {
            alpha *= 0.5; // エネルギーが上がってしまったら歩幅を半分に
        } else {
            alpha *= 1.1; // 順調に下がっていたら少し歩幅を広げる
        }
        pe_prev = e_pot;

        // 4. 座標の更新 (構造体 atoms の座標を直接いじる)
        for (int i = 0; i < NATOMS; i++) {
            double fx = atoms[i].fx;
            double fy = atoms[i].fy;
            double fz = atoms[i].fz;
            
            // クリップ処理：大きすぎる力は強制的にカット
        ...
        for (int j = 0; j < n_per_side && idx < NMOL; j++) {
            for (int k = 0; k < n_per_side && idx < NMOL; k++) {
                ...
                for (int s = 0; s < SITES_PER_MOL; s++) {
                    ...
                    // 初速度は0で初期化（実際はボルツマン分布を与えるのが一般的です）
