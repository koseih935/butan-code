...
#define THETA0  1.99         // 平衡結合角 (114度 = 約 1.99 rad) - 物理的なバネの自然長
#define THETA_EXP (100.0 * M_PI / 180.0) // acos近似の中心角 (100度)
...
// --- acosの9次多項式近似係数の初期化 ---
void init_acos_poly() {
    COS_THETA_EXP = cos(THETA_EXP);
    double x0 = COS_THETA_EXP;
    double sin_th0 = sin(THETA_EXP);
    double w = 1.0 - x0 * x0; // sin_th0^2 と等価

    // 係数計算 (テイラー展開の漸化式に基づく)
    acos_coeffs[0] = THETA_EXP;
...
void compute_angles() {
    ...
    for (int m = 0; m < NMOL; m++) {
        ...
        for (int i = 0; i < SITES_PER_MOL - 2; i++) {
            ...
            // --- acosを 100° = THETA_EXP 周りの9次多項式で近似 ---
            double dx = cos_theta - COS_THETA_EXP;
            ...
            // 力場エネルギーおよび微分計算には物理的な平衡角（THETA0 = 114°）を使用
